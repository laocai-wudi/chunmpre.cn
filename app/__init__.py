...
def create_app(config_name=None):
    ...
    def china_time_filter(dt, format_str='%Y-%m-%d %H:%M:%S'):
        ...
        注意：
        - 新数据（使用china_now()创建）已经是中国时区，直接显示
        - 旧数据（UTC时间）需要加上8小时转换为中国时区
        - 由于无法区分新旧数据，我们假设所有naive datetime都是中国时区（新数据）
        - 如果数据有tzinfo，则按实际时区转换
        """
        if dt is None:
            return None
        ...
        if dt.tzinfo is not None:
            ...
            # naive datetime：新数据已经是中国时区，直接格式化显示
            # 旧数据（UTC）会显示错误，但用户说旧数据不用管
